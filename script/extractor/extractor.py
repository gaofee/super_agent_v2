...
from core.utils import ensure_dir, run_local_command, write_text
...
class ScriptExtractor:
    ...
    def extract(self, input_video: Path, workdir: Path) -> tuple[Path, list[dict]]:
        ...
        if cmd_template:
            ...
            cmd = (
                f"ffmpeg -y -i {shlex.quote(str(input_video))} -vn "
                f"-acodec pcm_s16le -ar 16000 -ac 1 {shlex.quote(str(audio_path))}"
            )
